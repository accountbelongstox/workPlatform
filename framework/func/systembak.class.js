...
class systembakC{
    ...
    clearBakDir(){
        ...
        for(let i=0;i<clearDir.length;i++){
            that.o.tool.file.deletedir(clearDir[i]);
        }
    }
    ...
    getBakDirs(){
        let
            that = this,
            _bakDirs = that.o.node.fs.readdirSync(that.option.basicSystemBakDir),
            bakDirs = []
        ;
    ...
    isBakDir(dir){
        ...
        if(isBakDirReg.test(dir)){
            if(that.o.node.fs.existsSync(that.o.node.path.join(that.option.basicSystemBakDir,dir))){
                return true;
            }
        }
        return false;
    ...
    sortBakDirs(dirs){
        ...
        for(let i=0;i<sortDirs.length;i++){
            ...
            let _clearDir = that.o.node.path.join(that.option.basicSystemBakDir,folderName);
    ...
    pathMatchParse(pathAddress,ignore){
        ...
        if( pathAddress.includes("*") ){
            ...
            let
                ...
                backupFolderItemReg = new RegExp( that.o.tool.string.stringWildcardRegularized(pathAddress),"ig"),
                oneAddressMatchChar = "",
            ...
            addressArray.forEach( (oneAddress) => {
                if( oneAddress.includes("*") ){
                    ...
                    (!waitScanFolders) ? (waitScanFolders = oneAddress) : (waitScanFolders = that.o.node.path.join(waitScanFolders,oneAddress));
                }
            });
            if( that.o.node.fs.existsSync(waitScanFolders) && (scanFolders = that.o.node.fs.readdirSync(waitScanFolders)) ){
                let
                    folderReg = new RegExp(that.o.tool.string.stringWildcardRegularized(oneAddressMatchChar),"ig")
                ;
                scanFolders.forEach( (fold) => {
                    if( fold.match(folderReg) && that.checkIsNotIgnore(ignore,fold) ){
                        pathAddressArray.push( that.o.node.path.join(waitScanFolders ,fold) );
                    }
                });
            }
    ...
    */
    backupUserData(callback){
        ...
        ;
        //提示
        that.o.tool.console.success(`Start backup ${that.option.execType} ...`);
        //创建新的备份文件夹
        that.o.tool.file.mkdirSync(that.option.backupDir);
        //清空没用的空的备份文件夹
        that.o.tool.file.deleteChildrenEmptyDir(that.option.basicSystemBakDir);
        /**
         * @tools 备份系统环境变量
         */
        let
            Environments = that.option.conf.extend[that.option.execType].Environment.EnvName,
            backupConfigJSON = that.o.node.path.join(that.option.backupDir,"Environments/"+that.option.conf.extend[that.option.execType].Environment.backupConfig),
            EnvironmentJsonContent = {}
        ;
        Environments.forEach((EvnName,i)=>{
            let
                queryName = `%${EvnName}%`,
                result = that.o.tool.func.execSync(`echo ${queryName}`).join('\n')
            ;
            if(result === queryName){
                result = ``;
            }
        ...
        that.o.tool.file.writeFileSync(backupConfigJSON,JSON.stringify(EnvironmentJsonContent));
        //提示
        that.o.tool.console.success(that.option.backupDir);
        //计算出需要备份的文件目录
        that.getBackupsFolders(that.option.conf.extend[that.option.execType].localBackupScope,(needBackupFolders)=>{
            needBackupFolders.forEach((backupOneFolder)=>{
                let
                    ...
                    backupOneDir = that.o.node.path.join(that.option.backupDir,pathName)
                ;
                that.o.tool.file.node_copy(backupOneFolder,backupOneDir,(data)=>{
    ...
    */
    backupProgram(){
        ...
        that.getBackupsFolders(that.option.conf.extend[that.option.execType].localBackupScope,(needBackupFolders)=>{
            needBackupFolders.forEach((backupOneFolder)=>{
                let
                pathName = backupOneFolder.replace(/^.+?[\\\/]/,'').replace(/[\/|\\]+$/,''),
                backupOneDir = that.o.node.path.join(that.option.backupDir,pathName+".7z")
                ;
        ...
        (function executeZip(len){
            if(len < that.option.backupProgramList.length){
                ...
                ;
                that.o.tool.console.success(`Backup program [${len+1}/${that.option.backupProgramList.length}] : ${backupOne.file} to ${backupOne.target}`);
                that.o.tool.module.runModule(`command`,ZipSourceTarget,()=>{
                    executeZip(++len);
                });
            }
        })(0);
    ...
    getCurrentSystemOsIOSFile(){
        let
            that = this,
            sourceDir = that.o.config.platform.base.sourceDir.thisOS,
            thisOS = that.o.node.fs.readdirSync(sourceDir),
        ...
        OSName = that.o.node.path.join(sourceDir,OSName);
        return OSName;
    }
    ...
    initnodejs(){
        let
            that = this,
            apps = that.o.path.apps,
            nodeJsPath = that.o.node.path.join(apps,"nodejs/"),
        ...
        that.o.tool.func.exec(requireInstall,(info)=>{
            that.o.tool.windows.setEvn({
                PATH:nodeJsPath
            });
    ...
    initWindows(){
        let
            that = this,
            base = that.o.config.platform.base,
            sourceDir = base.sourceDir,
            workDir = base.workDir,
            tmpDir = that.o.node.path.join(base.local.tmpDir,`.init.windows`),
            currentOs = that.getCurrentSystemOsIOSFile(),
            installNet45 = that.option.extend.extend.installNet45
        ;

        that.o.tool.module.runCommandModule([
            "zip",
            "xtmp",
            `file:"${currentOs}"`
        ],(zipInfo)=>{
            let
                sources_sxs = that.o.node.path.join(zipInfo.target,`sources/sxs`).replace(/\//ig,`\\`),
                installNet = [
            ...
            //添加右键菜单
            that.o.tool.windows.addRightMenuSync("CMD run as Administrator Here","cmd.exe /s /k pushd \\\"%V\\\"",{type:"base",runas:true});
            //设置 framework 下 apps 基本的PATH
            //NODE.js等
            let
                appsPath = that.o.path.apps,
                apps = that.o.node.fs.readdirSync(appsPath),
                envObject = {
                    path : []
                }
            ;
            apps.forEach((app)=>{
               let
                   appPath = that.o.node.path.join(appsPath,app)
                ;
                envObject.path.push(appPath);
            });
            that.o.tool.windows.setEvn(envObject);
            //添加自定的个性菜单

            return;
            //开始安装.net 4.5
            that.o.tool.console.success(`Start install .net 4.5...`);
            that.o.tool.func.exec(installNet,(err)=>{
                that.o.tool.console.success(`Install .net 4.5 finish...`);
                //开始添加右键菜单
            },true/*显示输出信息*/);
        });
