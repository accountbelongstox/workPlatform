class sqliteC{
    ...
    table(table){
        let
            that = this,
            databaseConfig = that.o.config.platform.core.database,
    ...
    sqlObjectParse(tables){
        let
            ...
            tableArr = []
        ;
        if(that.o.tool.array.isObject(tables)){
            for(let p in tables){
                tableArr.push({
                    name : p,
                    value : tables[p]
        ...
        let
            ...
            tableValues = (function valueParse(values){
                let
                    valuesArr = []
                ;
                if(that.o.tool.array.isObject(values)){
                    for(let p in values){
                        let
                            value = values[p]
                        ;
            ...
            tableValues = (function valueParse(values){
                ...
                if(that.o.tool.array.isObject(values)){
                    for(let p in values){
                        ...
                        ;
                        if(!value)value=``;
                        value = that.o.tool.string.sqlParse(value);
                        valuesArr.push(`\`${p}\`='${value}'`);
                    }
                }else if(typeof values === "string"){
                    if(values){
    ...
    addValueParse(arr){
            ...
            tableValues = (function valueParse(values){
                ...
                if(that.o.tool.array.isObject(values)){
                    ...
                    if(values){
                        ...
                        ;
                        value = that.o.tool.string.trimX(value);
                        valuesArr.push({
                            key,
                            value
                        });
        ...
        values.forEach((value,index)=>{
            values[index] = that.o.tool.string.sqlParse(value);
        });
