        ...
        (function exec_cmd(len){
            if(len  >= command.length){
                ...
                let
                    cmd = command[len]
                ;
                that.common.node.child_process.exec(cmd, { encoding: binaryEncoding }, function(err, StdOpt, stderr){
                    StdOpt =  that.common.node[`iconv-lite`].decode(Buffer.alloc(StdOpt.length,StdOpt, binaryEncoding), encoding);
                    stderr =  that.common.node[`iconv-lite`].decode(Buffer.alloc(stderr.length,stderr, binaryEncoding), encoding);
                    output.push(StdOpt);
    ...
    */
    execSync(command){
        ...
        for(let i=0;i<command.length;i++){
            let cmd = command[i];
            try{
                let e = that.common.node.child_process.execSync(cmd, { encoding: binaryEncoding });
                e = e.toString("utf8");
                e = that.common.core.string.trim(e,[`\n`,`\r`]);
                console.log(e);
                output.push(e);
            }catch(err){
                console.log("error:");
        ...
        if(command.length > 0 ){
            (function spawn_cmd(i){
                ...
                splitCmds.forEach((cmdOne)=>{
                    ...
                    if(_cmdOne){
                        _cmdOne = that.common.core.string.trim(_cmdOne);
                        cmds.push(_cmdOne);
                    }
                });
                let
                    ...
                    parmas = cmds,
                    c = that.common.node.child_process.spawn(command_c,parmas)
                ;
        ...
        (function spawn_cmd(i){
            if(i >= command.length){
                ...
                let
                    ...
                    parmas = cmd.splice(1)
                ;
                that.common.node.child_process.execFile(command_c,parmas, (error, stdout, stderr) => {
                    if (error) {
                        console.log(error);
                    }
                    if (stderr) {
        ...
        (function spawn_cmd(i){
            if(i >= command.length){
                ...
                let
                    ...
                    parmas = cmd.splice(1)
                ;
                that.common.node.child_process.spawn(command_c,parmas);
                spawn_cmd(++i);
            }
        })(0);
    }
        ...
        if(!paramName && paramName !== 0){//如果不传该值,则返回一个带有各类方法的类.!=0 防止正好取下标为0的参数。0刚好被识别为false
            ...
            for(let i=0;i<opt.length;i++){
                ...
                _vTnameNotDir = function(){
                    //排除该参数为一个目录
                    let Dir = that.common.node.path.parse(_vTname);
                    if(Dir.root && Dir.dir){
                        return false;
                    }
                    return true;
