class sqliteC{
    ...
    sqlObjectParse(tables){
        let
            ...
            tableArr = []
        ;
        if(that.load.module.array.isObject(tables)){
            for(let p in tables){
                tableArr.push({
                    name : p,
                    value : tables[p]
        ...
        let
            ...
            tableValues = (function valueParse(values){
                let
                    valuesArr = []
                ;
                if(that.load.module.array.isObject(values)){
                    for(let p in values){
                        let
                            value = values[p]
                        ;
                ...
                }else if(that.load.module.array.isArray(values)){
            ...
            tableValues = (function valueParse(values){
                ...
                if(that.load.module.array.isObject(values)){
                    for(let p in values){
                        ...
                        ;
                        if(!value)value=``;
                        value = that.load.module.string.sqlParse(value);
                        valuesArr.push(`\`${p}\`='${value}'`);
                    }
                }else if(typeof values === "string"){
                    if(values){
    ...
    addValueParse(arr){
            ...
            tableValues = (function valueParse(values){
                ...
                if(that.load.module.array.isObject(values)){
                    ...
                    if(values){
                        ...
                        ;
                        value = that.load.module.string.trimX(value);
                        valuesArr.push({
                            key,
                            value
                        });
        ...
        values.forEach((value,index)=>{
            values[index] = that.load.module.string.sqlParse(value);
        });
