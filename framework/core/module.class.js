...
class moduleC{
	...
	constructor(common){
        ...
        common.get_tools('ddrun.public.tools');
        common.get_tools('module');
    ...
    runModule(moduleTypeName,option={},callback=null){
        ...
        (function runModuleListFunc(moduleLen){
            ...
            let
                ...
                argv = that.common.tools.module.optionToArgv(option),
                params = that.common.core.func.getParam(argv),
                //判断本次参数是否满足基本条件,指必须的参数都要有值
                //如果参数有多个,则根据构架,会返回一个数组,并依次由远而近的执行
                FullParams = that.common.tools.ddrunPublicTools.isFullParams(argv,params)
            ;
            //将params作为值通过common传入到即将调用的模块
            that.common.params = params;
            //必须的参数不全
            //此参数不成立
            if(FullParams === false){
            ...
            (function RunStartModule(len,classCount,funcCount/*用来统计执行了几次文件*/){
                ...
                let
                    ...
                    _extend_class = that.common.node.path.join(that.common.core.appPath[`${moduleTypeName}_modules`],paramName+`/index.class.js`),
                    beforeFuncName = ``,
                    paramsGroup = that.common.tools.module.paramsGroup(conf)
                ;
                ...
                */
                ...
                //由循环中判断是否有扩展参数类
                if(that.common.node.fs.existsSync( _extend_class )){
                    let
                    ...
                    ;
                    _extend_class_Func = new _extend_class_Func(that.common);
                    //给该方法common的值
                    _extend_class_Func.common = that.common;
                    //给该方法option
                    ...
                    */
                    if(_extend_class_Func[that.option.defaultRun]){
                        that.common.core.console.success(`default : ${paramName}/${that.option.defaultRun}:`);
                    ...
                    if(beforeFuncName && _extend_class_Func[beforeFuncName]){
                        that.common.core.console.success(`define : ${paramName}/${beforeFuncName}:`);
                        _extend_class_Func[beforeFuncName]( _callbak );
                    }
                    if(funcCount === 0 && len === (FullParams.length -1)){
                        that.common.core.console.error(`Not find function: ${that.option.defaultRun}\n`);
                    }
                }else{
                    //如果没有任何一个入口类,则提示
                    if(classCount === 0 && len === (FullParams.length -1)){
                        that.common.core.console.error(`Not find class: ${_extend_class}\n`);
                    }
                    RunStartModule(++len,++classCount,++funcCount);
                }
            })(0,0,0);
    ...
    */
    getModule(moduleTypeName,option={},callback=null){
        ...
        return (function runModuleListFunc(moduleLen){
            ...
            ;
            if(!newModule){
                that.common.core.console.error(`\nNot find module.`);
                return null;
            }
            ...
            let
                ...
                //如果参数有多个,则根据构架,会返回一个数组,并依次由远而近的执行
                FullParams = that.common.tools.ddrunPublicTools.isFullParams(argv,params),
                moduleName = params.contain(that.common.tools.module.getCommandModuleNames()),
                //由于获取时只能获取一个
                //此变量用于存放过滤后的模型对象
                FullParam = null
            ;
    ...
    */
    runEletornModule(option={},callback=null){
        ...
        ;
        let 
            modulePath = that.node.path.join(that.common.core.appPath.electron_modules,module+"/index.class.js"),
