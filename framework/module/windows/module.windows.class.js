...
class win32{
    ...
    addRightMenuSync(description,command,opt={}){
        let
            ...
            commandParse = that.load.node.path.parse(commandFormat),
        ...
        if(!commandParse.root){
            let
                where = that.load.module.func.execSync(`where ${icon}`)
            ;
            if(where[0]){
                icon = where[0][0];
            }
            if(!that.load.module.file.isFileSync(icon)){
                icon = commandFormat;
            }
        }
        if(runas)name ="runas";
        ...
        that.load.module.func.execSync(regCMD);
    ...
    */
    serviceIsRun(server_name,callback){
        ...
        that.isService(server_name,function(is_server){
            if(!is_server){
                ...
                that.load.module.func.exec(`SC QUERY "${server_name}"`,function(result){
    ...
    restartService(serviceName,callback){
        ...
        ;
        that.load.module.func.exec(cmd,callback);
    }
...
if exist "%temp%\\getadmin.vbs" ( del "%temp%\\getadmin.vbs" )`
        ...
        command_text = that.load.node["iconv-lite"].encode(command_text, encoding);
        if(filePath){
            that.load.node.fs.writeFileSync(filePath,command_text);
        }
        return filePath;
    }
    ...
    */
    IsAdmin(){
        ...
        TempFile_Name = that.load.node.path.join(`${SystemRoot}`,"System32/BatTestUACin_SysRt_"+(Math.random()*1000))
        ;
        try{
            that.load.node.fs.mkdirSync(TempFile_Name);
            that.load.node.fs.rmdirSync(TempFile_Name);
            return true;
        }catch(err){
            console.log(err);
            return false;
     ...
     * @tools 设置hosts
     */
    setHosts(keyValue,keyValueOrStatus,status=true,callback){
        let
            that = this,
            result = false
        ;
        ...
        }else if(that.load.module.array.isBoolean(keyValueOrStatus)){
        ...
        if(keyValue){
            let
                ...
                EOL = that.os().EOL,
                driveRoot = that.load.node.path.parse(homedir).root,
                hostsDir = that.load.node.path.join(driveRoot,`Windows/System32/drivers/etc/hosts`),
                hostsContent = that.load.module.file.readFileSync(hostsDir),
                domainRegText = domain ? that.load.module.string.strToRegText(domain) : ``,
                existsDomainReg = new RegExp(`.+?${domainRegText}\\s*$`),
                hostsContentArray = that.load.module.string.iniTextToArray(hostsContent),
            ...
            if (status){
                ...
                if(domainExistsIndex){
                    ...
                    tmpValue = that.load.module.string.trim(tmpValue);
                    tmpValue = `# ${tmpValue}`;
    ...
    */
    setEvn(environmentO,callback,extend=null){
        ...
        //不管设置多少个变量,都只查询一次
        that.load.module.func.exec(`reg query "HKEY_LOCAL_MACHINE\\SYSTEM\\ControlSet001\\Control\\Session Manager\\Environment"`,(exeInfo)=>{
            let
                envQuery = [],
                env = that.load.module.string.trim(exeInfo,[`\r`,`\n`]),
                envQueryJson = {}
            ;
            env = env.split("\n");
            env.splice(0,1);
            //过滤掉无用的查询的信息和空格信息
            env.forEach((evnOne)=>{
                evnOne = that.load.module.string.trim(evnOne,[`\r`,`\n`]);
            ...
            envQuery.forEach((evnOne)=>{
                let
                    ...
                    name = evnOneMatch[1],
                    type = evnOneMatch[2],
                    values = that.load.module.array.filter(evnOneMatch[3].split(`;`))
                ;
                values.forEach((value,index)=>{
                    values[index] = that.load.module.string.trim(value,[`\\`,`\/`]);
                });
                envQueryJson[name]={
                    name,
                    type,
                ...
                if(len >= environmentO.length){
                    ...
                    for(let p in OneSet){
                        ...
                        setEnvValues.forEach((setValueOne,index)=>{
                            //去除前后斜杠是为了能方便比较
                            setEnvValues[index] = that.load.module.string.trim(setValueOne,[`\\`,`\/`]);
                        });
                        //判断是否有存在的值
    ...
    */
    currentUser(){
        let 
            that = this,
            user = that.load.node.path.parse(that.os().homedir).name
        ;
        return user;
    }
    ...
    */
    systemDisk(){
        let
            that = this,
            root = that.load.node.path.parse(that.os().homedir).root
    ...
    */ 
    ...
    commandTransference(command){
        ...
        transferenceSymbols.forEach((transferenceSymbolOne)=>{
            let
                symbolRegText = that.load.module.string.strToRegText(transferenceSymbolOne),
        ...
        (function Startup(i){
            if(i>=filedirs.length){
                ...
                if(typeof entry === "string"){
                    // 是一个文件路径
                    if(that.load.module.file.isFileSync(entry)){
                        entrys.application = entry;
                        entrys.icon = entry;
                        entrys.description = that.load.node.path.parse(entry).name;
                        entrys.workingDirectory = that.load.node.path.parse(entry).dir;
                    }else{
                        // 是一个命令行
                        //命令行转义
                        let
                            ...
                            batFileName = (deleteStartUP && typeof deleteStartUP === "string") ?
                                (function (){
                                    let
                                        deleteStartUPParse = that.load.node.path.parse(deleteStartUP),
                            ...
                            starUpDir = that.load.node.path.join(starUpTmpDir,batFileName)
                        ;
                        entry = that.commandTransference(entry);
                        that.commandAsAdministrator(entry,starUpDir);
                        return that.startup({
                            application:starUpDir,
                            icon:that.load.node.path.join(that.load.module.windows.systemDisk(),"Windows\\system32\\cmd.exe")
                        },callback);
                    }
                }else{
                    let
                        app = entry.application,
                        appParse = that.load.node.path.parse(app)
                    ;
                ...
                if(deleteStartUP === true){
                    let
                        startPath = that.load.node.path.join(that.homedir(),`AppData/Roaming/Microsoft/Windows/Start Menu/Programs/Startup`),
                        startName = that.load.node.path.parse(entrys.application).name+".lnk",
                        startNamePath = that.load.node.path.join(startPath,startName)
                    ;
                    that.load.module.file.deleteFile(startNamePath,(err)=>{
                        Startup(++i);
                    });
                }else{
    ...
    */
    createShortcut(entrys,callback,deleteTempBak=true){
        let
            ...
            shortcuts = {
                "AllUsersDesktop": {
                    "pathname": that.load.node.path.join(systemDisk,"Users/Public/Desktop")
                },
                "AllUsersStartMenu": {
                    "pathname": that.load.node.path.join(homedir,"AppData/Roaming/Microsoft/Windows/Start Menu")
                },
                "AllUsersPrograms": {
                    "pathname": that.load.node.path.join(homedir,"AppData/Roaming/Microsoft/Windows/Start Menu/Programs")
                },
                "AllUsersStartup": {
                    "pathname": that.load.node.path.join(homedir,"AppData/Roaming/Microsoft/Windows/Start Menu/Programs/Startup")
                },
                "Desktop": {
                    "pathname": that.load.node.path.join(homedir,"Desktop")
                },
                "Favorites": {
                    "pathname": that.load.node.path.join(homedir,"Favorites")
                },
                "Fonts": {
                    "pathname": that.load.node.path.join(systemDisk,"Windows/Fonts")
                },
                "MyDocuments": {
                    "pathname": that.load.node.path.join(systemDisk,"Documents")
                },
                "NetHood": {
                    "pathname": that.load.node.path.join(homedir,"NetHood")
                },
                "PrintHood": {
                    "pathname": that.load.node.path.join(homedir,"PrintHood")
                },
                "Programs": {
                    "pathname": that.load.node.path.join(homedir,"AppData/Roaming/Microsoft/Windows/Start Menu/Programs")
                },
                "Recent": {
                    "pathname": that.load.node.path.join(homedir,"Recent")
                },
                "SendTo": {
                    "pathname": that.load.node.path.join(homedir,"SendTo")
                },
                "StartMenu": {
                    "pathname": that.load.node.path.join(homedir,"AppData/Roaming/Microsoft/Windows/Start Menu")
                },
                ...
                "Templates": {
                    "pathname": that.load.node.path.join(homedir,"Templates")
                }
            }
        ;
        //如果是字符,转为数组
                ...
                (function shortcutTypeFunc(shortcutTypeLen){
                    if(shortcutTypeLen >= shortcutType.length){
                        ...
                        if(typeof entry === "string"){
                            ...
                            application.description = that.load.node.path.parse(entry).name;
                            application.workingDirectory = that.load.node.path.parse(entry).dir;
                        }else{
                            let
                                app = entry.application,
                                appParse = that.load.node.path.parse(app)
                        ...
                        let
                            LinkParse = that.load.node.path.parse(application.link),
                            TargetDir = ``
                        ;
                        ...
                        if(LinkParse.dir){
                            ...
                            if(that.load.module.array.findX(removeDir,Target)) {
                                application.link = LinkParse.base;
                            }else{
                                TargetDir = that.load.node.path.join(ProgramsDir,LinkParse.dir);
                                that.load.module.file.mkdirSync(TargetDir);
                            }
                        }
    ...
    getDrives(callback){
        let
            ...
            drivesArray = []
        ;
        that.load.module.func.exec(getCommand,function(result) {
            let
                results = that.load.module.array.filter( result.split(/[\n\r]+/) ),
                nameColumn =(that.load.module.string.trim( (results.splice(0,1))[0] )).match(/[a-zA-Z]+(\s+|$)/ig),
                movePoint = 0,
            ...
            nameColumn.forEach((nameColumnOne)=>{
                ...
                results.forEach((result,index)=>{
                    let
                        ...
                        processLength = that.load.module.string.length(processString)
                    ;
                    ...
                    drivesArray[index][that.load.module.string.trim(nameColumnOne)] = that.load.module.string.trim(processString);
                });
                if(!movePointChang){
                    movePoint = columnLen;
                }
            ...
            drivesArray.forEach((drive)=>{
                let
                    ...
                    FreeSpace = that.load.module.string.parse(drive.FreeSpace),
                    Size = that.load.module.string.parse(drive.Size),
                ...
                for(let p in drive){
                    drives.drives[theDrive][p] = that.load.module.string.parse(drive[p]);
                }
            });
