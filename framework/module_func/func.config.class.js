class configC{
    ...
    GetVersionFull(installDir){
        let 
			...
			reg = /\.*/ig
        ;
        if(!that.load.module.file.isPath(installDir)){
            installDir = that.load.module_func.install.getSoftInfo(installDir).applicationDir
		}
		let
            versions = that.load.node.fs.readdirSync(installDir)
		;
        versions.forEach((version)=>{
            if(reg.test(version)){
                let
                    apache = that.load.node.path.join(installDir,version)
                ;
                if(that.load.module.file.isDirSync(apache)){
                    r.push(apache);
                }
            }
        });
    ...
    GetPHPVersion(phpVersionFull){
        let
            that = this,
            phpVersionsFullPath = that.load.module_func.config.GetVersionFull("php"),
            phpVersions = []
        ;
        phpVersionsFullPath.forEach((phpVersion)=>{
            let
                ...
                conf = that.load.node.path.join(phpVersion,`php.ini`),
				tmpJson = {}
            ;
            tmpJson[version]={
            	base:that.load.node.path.parse(phpVersion).base,
            	version,
                dir:phpVersion,
                conf
            };
        ...
        if(phpVersionFull){
            ...
            if(phpVersions[phpVersionFull]){
                ...
                that.load.module.console.error(`Not find php version in ${phpVersion}`);
                return null;
            }
		}else{
	...
	GetApacheMultiplePHPsApi(apacheVersion="2.4"){
    	let
			that = this,
			phpVersions = that.load.module_func.config.GetPHPVersion(),
            multiPHP = []
		;
        ...
        phpVersions.forEach((versionJson)=>{
            for(let p in versionJson){
                let
                    ...
                    //需要文件存在时才生成配置
                    FCgiD = that.load.module.file.isFilesSync(PHPCgiExe) ? `<IfModule fcgid_module>
	...
	FcgidWrapper "${PHPCgiExe}" .php
                ...
                switch (phpVersion) {
                    case "52":
                        sApi = that.load.module.file.isFilesSync(PHPts_dll,moduleFileDir) ? `LoadFile "${PHPts_dll}"
...
<IfModule php5_module>
	...
	AddType application/x-httpd-php .php .phtml
                        ...
                        break;
                    default:
                        sApi = that.load.module.file.isFilesSync(PHPts_dll,PHPLibSsh2_dll) ? `LoadFile "${PHPLibSsh2_dll}"
LoadFile "${PHPts_dll}"
...
<FilesMatch "\\.php$">
    ...
    formatApacheVersion(version="2.4"){
        let
            that = this
        ;
        version = that.load.node.path.parse(version).base;
            ...
            for(let o in PHPObjectOne){
                ...
                for(let u in PHPOne){
                    let
                        ...
                        confPath = that.load.node.path.join(version,sourceConf)
                    ;
                    phpMulti[o] = {
                        version : o,
                        conf : confPath,
                    ...
                    if(!that.load.module.file.isFileSync(confPath) && force){
                        that.load.module.console.info(`Create configuration file => ${confPath}`,4);
                        that.load.module.file.writeFileSync(confPath,multiPHPOne);
                    }else{
                        if(waring)that.load.module.console.waring(`Configuration file exists => ${confPath}`);
                    }
                }
            }
        }
    ...
    SetIni(option,callback,sourceIniText=null/*配置文件内容,每次队列只执行一次,且优先读取模版内容*/,debug=false){
        let
            ...
            iniDefaultPath = null,
            optionFast = option[0],
            EOL = that.load.module.windows.os().EOL,
            //此处3个变量需要在最顶上获取
            //因为是全局共用
            //1.例子配置文件
            ...
            annotationRegText = that.load.module.string.strToRegText(annotation),
            childrenSplit = ` -> `,
            closedSplit = ` <-> `
        ;
        if(!confName || !iniPath){
            that.load.module.console.error(`Not find config name or ini path.`);
            if(callback)callback(sourceIniText);
        }else{
            if(!iniDefaultPath){
                iniDefaultPath = that.load.node.path.join(iniPath,confName);
                that.load.module.console.success(`Set ini in ${iniDefaultPath} `);
            }
            if(!sourceIniText){
                // 从模版读取配置
                // 备份至模版,此配置文件只读取一次
...
/*            if(debug){
            ...
            //获得配置文件转成的数组
            sourceIniArray = that.load.module.string.iniTextToArray(sourceIniText,annotation,filter);
            ...
            (function startSetINI(len){
                if(len >= option.length){
                    ...
                    that.load.node.fs.writeFileSync(iniDefaultPath,sourceIniText,{encoding: 'utf8'});
                    if(callback)callback(sourceIniText);
                }else{
                    let
                        opt = option[len],
                    ...
                    if(tag){
                        // 指定了标签 [scope]
                        tagStartText = tagLeft+that.load.module.string.trim(tag)+tagRight;
                        tagEndText = tagLeft;
                    }
                    // 再次处理源配置文件的分割数组长
                    // 处理 aaa -> bbb形式
                    if(isChildrenKey){
                        // 指定了标签 [scope]
                        key = that.load.module.string.trim(childrenKey[1]);
                        tagStartText = tagLeft+that.load.module.string.trim(childrenKey[0])+tagRight;
                        tagEndText = tagLeft;
                    }
                    if(isCloseKey){
                        // 处理闭合配置 <a> ... </a>
                        ...
                        tagStartText = that.load.module.string.trim(closedKey[0]);
                        tagEndText = that.load.module.string.trim(closedTag);
                    }
                    let
                        defineTagRegT = that.load.module.string.createRegExp(tagStartText,`i`),
                        // 用来查找标签的配置段范围
                        isTagConfReg = that.load.module.string.createRegExp(tagEndText,`i`),
                    ...
                    if(!queryScopeIs && !checkKey){
                        ...
                        that.load.module.console.info(`\t( ${showKey} ) : >>>>>> ${value} add success!`,3);
                        ...
                        if(valueExistsIndex !== null){
                            let
                                ...
                                newValue = (function (){
                                    ...
                                    if(valueTrim){
                                        result = that.load.module.string.trim(newValue);
                                    }
                                    return result;
                                })()
                            ;
    ...
    getKeyReg(key,value,assignmentSymbol,annotationRegText,multiKey){
        let
            that = this,
            keyQueryText,
            keyRegText = that.load.module.string.strToRegText(key),
            keyValueReg = null
        ;
    ...
    getKeyValueReg(key,assignmentSymbol,annotationRegText,multiKey,value){
        ...
        let
            that = this,
            keyRegText = that.load.module.string.strToRegText(key),
            valueRegText = that.load.module.string.strToRegText(value),
            valueTexts = [],
            multiKeyRegArray = []
        ;
        ...
        if( !(/\s+/ig.test(assignmentSymbol))){
            assignmentSymbol = that.load.module.string.trim(assignmentSymbol);
        }
        assignmentSymbol = that.load.module.string.strToRegText(assignmentSymbol);
        if(keyRegText){
    ...
    getSetIniKV(key,keyPrefix,value,assignmentSymbol,valueSymbol,changQuotePath,annotation,queryValue=``){
        ...
        let
            ...
            KV = ``,
            valueX = value,
            assignmentSymbolTrim = that.load.module.string.trim(assignmentSymbol)
        ;
        value = that.load.module.string.trim(value,[],true);
        if(key){
            //如果有空格,或非数字开头
            ...
            if(changQuotePath && queryValue){
                ...
                if(assignmentSymbol){
                    ...
                    ;
                    queryValueParse = that.load.module.string.trim(queryValueParse);
                    queryValueMatch = queryValueParse[queryValueParse.length-1];
                }else{
                    queryValueMatch = valueX;
                }
                valueX = that.load.module.file.queryPathJoin(value,queryValueMatch);
            }
    ...
    createSetIniValueDir(value,valueIsDirAndCreate,valueIsFileAndCreate,valueIsFileAndCreateDir,checkKey,isQueryValue){
        ...
        if(!checkKey || (checkKey && isQueryValue)){
            value = that.load.module.string.trim(value);
            // 值是一个目录并且创建
            if(valueIsDirAndCreate){
                that.load.module.file.mkdirSync(value);
            }
            // 值是一个文件并且创建
            if(valueIsFileAndCreate){
                that.load.module.file.touchSync(value);
            }
            // 值是一个文件并且创建上级目录
            if(valueIsFileAndCreateDir){
                that.load.module.file.mkFileDirSync(value);
            }
        }
    }
    ...
    GetIniInTemplateAndBackupToTmplate(iniDir,exampleConf,confName){
        let
            that  = this
        ;
        if(!that.load.module.func.exists(exampleConf)){
            exampleConf = [confName];
        }
        if(!confName){
            confName = that.load.node.path.parse(iniDir).base;
        }
        if(that.load.module.file.isFileSync(iniDir)){
            iniDir = that.load.node.path.join(iniDir,`../`);
        }
        let
            configContent = ``,
        ...
        allConfigs.forEach( (confNameOne)=>{
            let
                confPathOne = that.load.node.path.join(iniDir,confNameOne),
                templateConfDir = that.load.node.path.join(templateDir,confNameOne)
            ;
            //如果模版文件没有,则要复制过去
            if( !that.load.module.file.isFileSync(templateConfDir) ){
                //如果源配置存在,则要备份到模版
                if( that.load.module.file.isFileSync(confPathOne) ){
                    let
                        iniContentOne = that.load.module.file.readFileSync(confPathOne)
                    ;
                    that.load.module.file.writeFileSync(templateConfDir,iniContentOne);
                }
            }
        });
        //如果默认配置不存在,则要从开发配置复制过来
        if( !that.load.module.file.isFileSync(confNameTemplatePath) ){
            exampleConf.forEach((exampleConfOne)=>{
                let
                    exampleConfOnePath =that.load.node.path.join(templateDir,exampleConfOne)
                ;
                //第一个就被复制过来
                if( that.load.module.file.isFileSync(exampleConfOnePath) ){
                    configContent = that.load.module.file.readFileSync(exampleConfOnePath);
                    that.load.module.file.writeFileSync(confNameTemplatePath,configContent);
                    return configContent;
                }
            });
        }else{
            configContent = that.load.module.file.readFileSync(confNameTemplatePath);
        }
        return configContent;
    }
    /*
    ...
    */
    phpCurrentApplicationDir(){
        let
            that  = this,
            phpInfo = that.load.module_func.install.getSoftInfo("php"),
            //通过环境变量直接取得
            current = that.load.node.path.join(phpInfo.applicationDir,phpInfo.environmentVariable.path)
        ;
    ...
    installHttpdModule(apacheDir,key,callback,ConfigContent,status=true){
    	let
    	that = this,
    	installConfig = that.load.support.webserver.httpd.installModule[key]
    	;
        that.option.SetIniPublic= {
            path:apacheDir,
    	...
    	if(installConfig){
            ...
            if(status){
                ...
                (function startDownModule(len){
                    if(len >= remoteUrls.length){
                        that.load.module.console.error(`Download address failed : ${remoteUrls} ..`);
                        ...
                        that.load.module.network.downFile({
                            ...
                            if(!savepath){
                                ...
                                ;
                                //调用解压模块
                                that.load.run_module(`command`,opt,(exFile)=>{
                                    if(exFile.target){
                                        let
                                            soFile = `${key}.so`,
                                            mod_fcgid = that.load.module.file.queryFileSync(exFile.target,soFile),
                                            moduleDir = that.load.node.path.join(apacheDir,`modules/${soFile}`)
                                        ;
                                        setIniArray.push({
                                            value:moduleValue,
                                        });
                                        if(!that.load.module.file.isFileSync(moduleDir)){
                                            //将文件复制过去
                                            that.load.node.fs.createReadStream(mod_fcgid).pipe(that.load.node.fs.createWriteStream(moduleDir)).on('close',(err)=>{
                                        ...
                                        that.load.module.console.error(`Unzip failed: ${exFile} ..`);
                                        if(callback)callback();
                                    }
                                });
                            }
                ...
                that.load.module.console.error(`Disabled: ${key}`);
                setIniArray.push({
                    status,
                    value:moduleValue,
                });
    		...
    		that.load.module.console.error(`Not find LoadModule ${key} in support ..`);
    		if(callback)callback();
    	}
    }
}
