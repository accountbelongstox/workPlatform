...
class win32{
    ...
    addRightMenuSync(description,command,opt={}){
        let
            ...
            commandParse = that.o.node.path.parse(commandFormat),
        ...
        if(!commandParse.root){
            let
                where = that.o.tool.func.execSync(`where ${icon}`)
            ;
            if(where[0]){
                icon = where[0][0];
            }
            if(!that.o.tool.file.isFileSync(icon)){
                icon = commandFormat;
            }
        }
        if(runas)name ="runas";
        ...
        that.o.tool.func.execSync(regCMD);
    ...
    */
    serviceIsRun(server_name,callback){
        ...
        that.isService(server_name,function(is_server){
            if(!is_server){
                ...
                that.o.tool.func.exec(`SC QUERY "${server_name}"`,function(result){
    ...
    restartService(serviceName,callback){
        ...
        ;
        that.o.tool.func.exec(cmd,callback);
    }
...
if exist "%temp%\\getadmin.vbs" ( del "%temp%\\getadmin.vbs" )`
        ...
        command_text = that.o.node["iconv-lite"].encode(command_text, encoding);
        if(filePath){
            that.o.node.fs.writeFileSync(filePath,command_text);
        }
        return filePath;
    }
    ...
    */
    IsAdmin(){
        ...
        TempFile_Name = that.o.node.path.join(`${SystemRoot}`,"System32/BatTestUACin_SysRt_"+(Math.random()*1000))
        ;
        try{
            that.o.node.fs.mkdirSync(TempFile_Name);
            that.o.node.fs.rmdirSync(TempFile_Name);
            return true;
        }catch(err){
            console.log(err);
            return false;
    ...
    setHosts(keyValue,keyValueOrStatus,status=true){
        ...
        }else if(that.o.tool.array.isBoolean(keyValueOrStatus)){
        ...
        if(keyValue){
            let
                ...
                EOL = that.os().EOL,
                driveRoot = that.o.node.path.parse(homedir).root,
                hostsDir = that.o.node.path.join(driveRoot,`Windows/System32/drivers/etc/hosts`),
                hostsContent = that.o.tool.file.readFileSync(hostsDir),
                domainRegText = domain ? that.o.tool.string.strToRegText(domain) : ``,
                existsDomainReg = new RegExp(`.+?${domainRegText}\\s*$`),
                hostsContentArray = that.o.tool.string.iniTextToArray(hostsContent),
            ...
            if (status){
                ...
                if(domainExistsIndex){
                    ...
                    tmpValue = that.o.tool.string.trim(tmpValue);
                    tmpValue = `# ${tmpValue}`;
                    hostsContentArray[domainExistsIndex] = tmpValue;
                }
            }
            hostsContent = hostsContentArray.join(EOL);
            that.o.tool.console.success(`Set hosts in => ${keyValues} `);
            that.o.tool.file.writeFileSync(hostsDir,hostsContent);
            return true;
        }else{
            that.o.tool.console.error(`Not find set Key or Value ...`);
            return false;
        }
    }
    ...
    */
    setEvn(environmentO,callback,extend=null){
        ...
        //不管设置多少个变量,都只查询一次
        that.o.tool.func.exec(`reg query "HKEY_LOCAL_MACHINE\\SYSTEM\\ControlSet001\\Control\\Session Manager\\Environment"`,(exeInfo)=>{
            let
                envQuery = [],
                env = that.o.tool.string.trim(exeInfo,[`\r`,`\n`]),
                envQueryJson = {}
            ;
            env = env.split("\n");
            env.splice(0,1);
            //过滤掉无用的查询的信息和空格信息
            env.forEach((evnOne)=>{
                evnOne = that.o.tool.string.trim(evnOne,[`\r`,`\n`]);
            ...
            envQuery.forEach((evnOne)=>{
                let
                    ...
                    name = evnOneMatch[1],
                    type = evnOneMatch[2],
                    values = that.o.tool.array.filter(evnOneMatch[3].split(`;`))
                ;
                values.forEach((value,index)=>{
                    values[index] = that.o.tool.string.trim(value,[`\\`,`\/`]);
                });
                envQueryJson[name]={
                    name,
                    type,
                ...
                if(len >= environmentO.length){
                    ...
                    for(let p in OneSet){
                        ...
                        setEnvValues.forEach((setValueOne,index)=>{
                            //去除前后斜杠是为了能方便比较
                            setEnvValues[index] = that.o.tool.string.trim(setValueOne,[`\\`,`\/`]);
                        });
                        //判断是否有存在的值
                        ...
                        if(existsEvnJson){
                            ...
                            //将值去重
                            setEnvValues = that.o.tool.array.unique(setEnvValues);
                            //得出最终要设置的值
                            setEnvValue = setEnvValues.join(`;`);
                            that.o.tool.console.info(`SET ENVIRONMENT ${setEnvName} => ${setEnvValue}\n`,4);
                            //检查是否有重复的值
                        }else{
                            ...
                            setEnvValues = that.o.tool.array.unique(setEnvValues);
                            setEnvValue = setEnvValues.join(`;`);
                            that.o.tool.console.info(`ADD ENVIRONMENT ${setEnvName} => ${setEnvValue}\n`,4);
                        }
                    ...
                    that.o.tool.func.exec(commands,()=>{
                        startSet(++len);
                    });
                }
            })(0);
    ...
    */
    currentUser(){
        let 
            that = this,
            user = that.o.node.path.parse(that.os().homedir).name
        ;
        return user;
    }
    ...
    */
    systemDisk(){
        let
            that = this,
            root = that.o.node.path.parse(that.os().homedir).root
    ...
    */ 
    ...
    commandTransference(command){
        ...
        transferenceSymbols.forEach((transferenceSymbolOne)=>{
            let
                symbolRegText = that.o.tool.string.strToRegText(transferenceSymbolOne),
        ...
        (function Startup(i){
            if(i>=filedirs.length){
                ...
                if(typeof entry === "string"){
                    // 是一个文件路径
                    if(that.o.tool.file.isFileSync(entry)){
                        entrys.application = entry;
                        entrys.icon = entry;
                        entrys.description = that.o.node.path.parse(entry).name;
                        entrys.workingDirectory = that.o.node.path.parse(entry).dir;
                    }else{
                        // 是一个命令行
                        //命令行转义
                        let
                            starUpTmpDir = that.o.node.path.join(that.o.config.platform.base.local.tmpDir,`.startup`),
                            batFileName = (deleteStartUP && typeof deleteStartUP === "string") ?
                                (function (){
                                    let
                                        deleteStartUPParse = that.o.node.path.parse(deleteStartUP),
                            ...
                            starUpDir = that.o.node.path.join(starUpTmpDir,batFileName)
                        ;
                        entry = that.commandTransference(entry);
                        that.commandAsAdministrator(entry,starUpDir);
                        return that.startup({
                            application:starUpDir,
                            icon:that.o.node.path.join(that.o.tool.windows.systemDisk(),"Windows\\system32\\cmd.exe")
                        },callback);
                    }
                }else{
                    let
                        app = entry.application,
                        appParse = that.o.node.path.parse(app)
                    ;
                ...
                if(deleteStartUP === true){
                    let
                        startPath = that.o.node.path.join(that.homedir(),`AppData/Roaming/Microsoft/Windows/Start Menu/Programs/Startup`),
                        startName = that.o.node.path.parse(entrys.application).name+".lnk",
                        startNamePath = that.o.node.path.join(startPath,startName)
                    ;
                    that.o.tool.file.deleteFile(startNamePath,(err)=>{
                        Startup(++i);
                    });
                }else{
    ...
    */
    createShortcut(entrys,callback,deleteTempBak=true){
        let
            ...
            shortcuts = {
                "AllUsersDesktop": {
                    "pathname": that.o.node.path.join(systemDisk,"Users/Public/Desktop")
                },
                "AllUsersStartMenu": {
                    "pathname": that.o.node.path.join(homedir,"AppData/Roaming/Microsoft/Windows/Start Menu")
                },
                "AllUsersPrograms": {
                    "pathname": that.o.node.path.join(homedir,"AppData/Roaming/Microsoft/Windows/Start Menu/Programs")
                },
                "AllUsersStartup": {
                    "pathname": that.o.node.path.join(homedir,"AppData/Roaming/Microsoft/Windows/Start Menu/Programs/Startup")
                },
                "Desktop": {
                    "pathname": that.o.node.path.join(homedir,"Desktop")
                },
                "Favorites": {
                    "pathname": that.o.node.path.join(homedir,"Favorites")
                },
                "Fonts": {
                    "pathname": that.o.node.path.join(systemDisk,"Windows/Fonts")
                },
                "MyDocuments": {
                    "pathname": that.o.node.path.join(systemDisk,"Documents")
                },
                "NetHood": {
                    "pathname": that.o.node.path.join(homedir,"NetHood")
                },
                "PrintHood": {
                    "pathname": that.o.node.path.join(homedir,"PrintHood")
                },
                "Programs": {
                    "pathname": that.o.node.path.join(homedir,"AppData/Roaming/Microsoft/Windows/Start Menu/Programs")
                },
                "Recent": {
                    "pathname": that.o.node.path.join(homedir,"Recent")
                },
                "SendTo": {
                    "pathname": that.o.node.path.join(homedir,"SendTo")
                },
                "StartMenu": {
                    "pathname": that.o.node.path.join(homedir,"AppData/Roaming/Microsoft/Windows/Start Menu")
                },
                ...
                "Templates": {
                    "pathname": that.o.node.path.join(homedir,"Templates")
                }
            }
        ;
        //如果是字符,转为数组
                ...
                (function shortcutTypeFunc(shortcutTypeLen){
                    if(shortcutTypeLen >= shortcutType.length){
                        ...
                        if(typeof entry === "string"){
                            ...
                            application.description = that.o.node.path.parse(entry).name;
                            application.workingDirectory = that.o.node.path.parse(entry).dir;
                        }else{
                            let
                                app = entry.application,
                                appParse = that.o.node.path.parse(app)
                        ...
                        let
                            LinkParse = that.o.node.path.parse(application.link),
                            TargetDir = ``
                        ;
                        ...
                        if(LinkParse.dir){
                            if(!ProgramsDir){
                                that.o.tool.console.error(`Not find ProgramsDir.`);
                                shortcut(++i);
                                return;
                            }
                            if(that.o.tool.array.findX(removeDir,Target)) {
                                application.link = LinkParse.base;
                            }else{
                                TargetDir = that.o.node.path.join(ProgramsDir,LinkParse.dir);
                                that.o.tool.file.mkdirSync(TargetDir);
                            }
                        }
                        ...
                        let
                            tmpDir = that.o.node.path.join(that.o.config.platform.base.local.tmpDir,`.shortcut`),
                            tmpVbs = that.o.node.path.join(tmpDir,`tmpStartup${Date.parse(new Date())}${Math.ceil(Math.random() * 10000)}.vbs`),
                            commands = [
                        ...
                        ;
                        that.o.tool.file.mkdirSync(tmpDir);
                        if( deleteTempBak )commands.push(`del /f /q "${tmpVbs}"`);
                        result.push(application);
                        that.o.tool.console.info(`create ${Target} to ${ProgramsDir}`,4);
    ...
    getDrives(callback){
        let
            ...
            drivesArray = []
        ;
        that.o.tool.func.exec(getCommand,function(result) {
            let
                results = that.o.tool.array.filter( result.split(/[\n\r]+/) ),
                nameColumn =(that.o.tool.string.trim( (results.splice(0,1))[0] )).match(/[a-zA-Z]+(\s+|$)/ig),
                movePoint = 0,
            ...
            nameColumn.forEach((nameColumnOne)=>{
                ...
                results.forEach((result,index)=>{
                    let
                        ...
                        processLength = that.o.tool.string.length(processString)
                    ;
                    ...
                    drivesArray[index][that.o.tool.string.trim(nameColumnOne)] = that.o.tool.string.trim(processString);
                });
                if(!movePointChang){
                    movePoint = columnLen;
                }
            ...
            drivesArray.forEach((drive)=>{
                let
                    ...
                    FreeSpace = that.o.tool.string.parse(drive.FreeSpace),
                    Size = that.o.tool.string.parse(drive.Size),
                ...
                for(let p in drive){
                    drives.drives[theDrive][p] = that.o.tool.string.parse(drive[p]);
                }
            });
