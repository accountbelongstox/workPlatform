...
class index {
    ...
    run(callback) {
        ...
        //需要下载的URL
        that.option.copyWebBaseDownloadUrl = that.load.params.get('url');
        //当前需要启动的线程数
        that.option.thread = that.load.params.get('thread',1);
        //正在下载线程数
        that.option.currentDownloadThread = 0;
        //单个文件请求超时时间
        that.option.timeout = that.load.params.get('timeout',30);
        //是否将base64转为图片,默认不转
        that.option.base64ToPng = that.load.params.get('base64',false);
        //下载层级
        that.option.level = that.load.params.get('level',3);
        //默认编码,不管指定什么编码,都会自动查找修正
        that.option.charset = that.load.params.get('charset',"utf8");
        ...
        */
        ...
        that.option.copyWebBasicSavePath = that.load.params.get('save',null,true);
        ...
        //url的基地址
        that.option.basicURL = that.load.module.network.formatUrl(that.option.copyWebBaseDownloadUrl );
        //域名协议
        that.option.protocol = that.load.node.url.parse(that.option.basicURL).protocol;
        //本次下载的基本保存地址
        firstSavaPath = that.load.module.network.URLGetLocalSaveAddress(that.option.copyWebBaseDownloadUrl ,that.option.copyWebBasicSavePath,true,false);
        that.option.basicSavePath = that.load.node.path.parse(firstSavaPath).dir;
    ...
    StartDownloadThisUrlSet(currentDowloadUrlObject){
        ...
        */
        ...
        if(!currentDowloadUrlObject.isStaticSource){
            that.load.module.network.HTMLGet({
    ...
    * */
    HTMLQueryCssJsImg(HTMLdata,currentDowloadUrlObject) {
        ...
        currentSaveDir = that.load.node.path.parse(currentSavePath).dir,
                    ...
                    needDeleteDocumentElement.forEach((needDeleteArry) =>{
                        for(let needDeleteEleTag in needDeleteArry){
                            ...
                            deleteOneArray.forEach( (deleteOne) =>{
                                let
                                queryTagText  = `\\<\\s*${HTMLTagName}.+?${needDeleteEleTag}\\s*\\=\\s*[\\'\\"]{1}.*${that.load.module.string.strToRegText(deleteOne)}.*[\\'\\"]{1}.+?\\>`,
        ...
        $cheerio = that.load.node.cheerio.load(HTMLdata);
        ...
        //同时减去一个线程计数
        //并同时启动一个新的下载分离器
        that.load.module.file.writeFile(currentSavePath,HTMLdata,that.option.charset);
    ...
    */
    ...
    save_source(src_save_path, http_src) {
        ...
        that.load.module.file.mkdirSync(img_save_path_parse.dir);
    ...
    */
    fieldsWithDomainGetAddresses(option){
        ...
        let
        privateQueryField = that.load.module.string.trim(queryField),
        //将原代码本地化后的代码
        ...
        if( !isDataBase64 ){
            ...
            mergeUrlObject = that.load.module.network.formatUrlAndGetIsSuper(privateQueryField,currentDowloadUrl);
            mergeUrl = mergeUrlObject.downloadUrl;
            isSuper = mergeUrlObject.isSuper;
            sourceUrlDownloadSavaPath = that.load.module.network.URLGetLocalSaveAddress(mergeUrl,null,true);
            ...
            let
            mergeUrlParse = that.load.node.url.parse( mergeUrl ),
            basicUrlParse = that.load.node.url.parse( that.option.basicURL )
            ;
            ...
            //将替换代码修改为路径 
            transformCode = that.load.node.url.parse(mergeUrl).pathname;

            //如果是顶级目录,则和根路径连接
            if(isSuper){
                transformCode = "/"+sourceUrlDownloadSavaPath.replace(/^\//,``);
                sourceUrlDownloadSavaPath = that.load.node.path.join( that.option.basicSavePath ,sourceUrlDownloadSavaPath);
            }else{
                if(!isRelativeUrl){
                    ...
                    sourceUrlDownloadSavaPath = that.load.node.path.join(currentSaveDir,sourceUrlDownloadSavaPath);
                }
            }
            ...
            if(checkDownloadLevel){
                ...
                while(level >= 0){
                    level -- ;
                    checkUrlLevel = that.load.node.path.join(checkUrlLevel,"../");
                }
                //超出层级
                pageLevelNotOverflow = /[\.\\\/]+$/.test(that.load.node.path.parse(checkUrlLevel).base);
            }
            ...
            if( that.option.base64ToPng  ){
                ...
                imageFileName = that.load.module.encrypt.md5(base64)+".png",
                base64Buffer = Buffer.alloc(base64.length,base64, 'base64')
                ;

                sourceUrlDownloadSavaPath = that.load.node.path.join(currentSaveDir,imageFileName);
                transformCode = `${imageFileName}`;

                if(Buffer.isBuffer(base64Buffer)){
                    that.load.module.file.writeFile(sourceUrlDownloadSavaPath,base64Buffer)
               }
            }
        }
