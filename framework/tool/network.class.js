class networkC{
    ...
    HTMLGet(option, fn) {
        ...
        protocol = that.o.node.url.parse(option.url).protocol.replace(/\:/ig,``),
        getFn,
        request_timer,
        req
        ;
    ...
    URLGetLocalSaveAddress(downloadUrl,basicSavePath,mkdir=false,returnDir=false){
        ...
        basicURLPaser = that.o.node.url.parse(basicURL),
        downloadUrlParse =  that.o.node.url.parse(downloadUrl),
        ...
        let
        savePath = that.o.node.path.join(basicSavePath, basicURLPaser.hostname),
        dir = ``
        ;

        savePath = that.o.node.path.join(savePath, urlPathName);
        dir = that.o.node.path.parse(savePath).dir;

        if(mkdir){
            that.o.tool.file.mkdirSync(dir);
        }else if(returnDir){
            return dir;
        }
        return savePath;
    ...
    */
    ...
    formatUrl(downloadUrl,basicURL=null,getUrlRoot=false,getSuper=false/*判断是否是根目录*/) {
        ...
        benchmarkUrlParse = that.o.node.url.parse(benchmarkUrl),
        benchmarkPathParse = that.o.node.path.parse(benchmarkUrl),
        ...
        let
        isDownloadUrlUrlParse = that.o.node.url.parse(downloadUrl)
        ;
    ...
    */
    ...
    isStaticSourceUrl(url){
        ...
        that = this,
        fileExts = [],
        ext = that.o.node.path.parse(url).ext.replace(/\./,'')
        ;
        for(let p in that.o.support.file){
            let 
            tmp = that.o.support.file[p]
            ;
            if('extend' in tmp){
                fileExts = fileExts.concat(tmp.extend)
            }
        }
        fileExts = that.o.tool.array.unique(fileExts);
    ...
    */
    downFile(option,fn=null){
        let 
            ...
            force = option["force"],
            fileName = that.o.node.path.parse(dowUrl).base,
            downUrlParse = that.o.node.url.parse(dowUrl),
            saveDir = that.o.node.url.parse(dowUrl).path,
            savePathDir = ""
        ;
        //如果没有指定存放地址,则下载将被存放到临时文件中
        if(!savePath){
            savePath = that.o.node.path.join(that.o.config.platform.base.local.tmpDir,".down")
        }
        
        if(!saveDir){
            saveDir = that.o.node.url.parse(dowUrl).pathname;
        }

        savePath = that.o.node.path.join(savePath,saveDir);
        //得到保存地址的父目录,以便于创建
        savePathDir = that.o.node.path.parse(savePath).dir;

        if(!that.o.tool.file.isFileSync(savePath) || force == true){

            //创建保存目录
            that.o.tool.file.mkdirSync(savePathDir);

            //创建文件流
            let stream = that.o.node.fs.createWriteStream(savePath);

            let options = {
                url:dowUrl,  
                encoding:null//当请求的是二进制文件时，一定要设置  
            }  
            that.o.node.request.get(options).on('response',function (response){//显示进度条  
                let proStream = that.o.node['progress-stream']({  
                ...
                that.o.node.request.get(options).pipe(proStream).pipe(stream).on("close", function (err) {
                    if(fn)fn(savePath);
