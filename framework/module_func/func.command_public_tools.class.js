class ddrunPublicTools{
	...
	*/
	paramsNotice(config){
		...
		;
        that.load.module.console.error(errorInfo);
	...
	*/
	showCommandHelp(fn_name,params){
		...
		if(!fn_name){
            ...
            let
				modules = that.load.core.path.getDir(`command_modules`),
                conf = this.o.config.framwork.modules.command
			;
            for(let p in modules){
            	let
					e = that.load.node.path.parse(modules[p]).base,
                    description = ""
				;
                console.log(`--${ e }`);
            	if(e in conf){
	...
	printCommandParamsNeedRequirement(config){
        ...
        for( let p in config ){
        	...
        	if(p == `mustParams` && sourceChildren){
                consoleType = 2;
		        that.load.module.console.info("* Necessary parameters :",consoleType);
        	}
        	if(p == `additionalParams` && sourceChildren){
                consoleType = 5;
                that.load.module.console.info("* Addition parameter :",consoleType);
        	}
        	...
        	if(p == `mustParams`  || p == `additionalParams` ){
	        	...
	        	for( let k in sourceChildren ){
                    ...
                    that.load.module.console.info(` ${before}${k}`,consoleType);
                    //如果可以一个参数
                    if(mustParamsIsOne && d.MustBeSatisfied){
                        that.load.module.console.info(MustBeSatisfiedInfo,8);
                    }
                    //没有标识可以单个参数
                    if(!mustParamsIsOne){
                        that.load.module.console.info(MustBeSatisfiedInfo,8);
                    }
                    that.load.module.console.info(`\tdescription : ${d.description}`,consoleType);
                    if(format){
                        that.load.module.console.info(format,consoleType);
                    }
	        	}
        	}
        }
