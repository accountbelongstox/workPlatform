        ...
        let
            ...
            domains = (function (){
                let
                    ...
                    domainTmpSouce = ``
                ;
                domainTmp = that.o.tool.string.trimX(domainTmp,`.`);
                ...
                domains = that.o.tool.array.unique(domains);
                that.o.tool.console.info(`create domain "${domains.join(` `)}"`,5);
                return domains
            })(),
            domain = domains.splice(0,1)[0],
            wwwroot = that.o.config.platform.base.workDir.wwwroot,
            webDir = that.o.node.path.join(wwwroot,domain),
            apacheVersions = that.o.func.config.GetVersionFull(`httpd`),
            o = {}
        ;
        apacheVersions.forEach((apache)=>{
            let
                PHPs = that.o.func.config.GetOrAddMultiPHPsApiAndFCgiDConf(apache,false,false),
                apacheVersion = that.o.func.config.formatApacheVersion(apache)
            ;
            for(let p in PHPs){
                let
                    PHP = PHPs[p]
