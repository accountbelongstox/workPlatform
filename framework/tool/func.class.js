        ...
        (function exec_cmd(len){
            if(len  >= command.length){
                ...
                let
                    cmd = command[len]
                ;
                that.o.node.child_process.exec(cmd, { encoding: binaryEncoding }, function(err, StdOpt, stderr){
                    StdOpt =  that.o.node[`iconv-lite`].decode(Buffer.alloc(StdOpt.length,StdOpt, binaryEncoding), encoding);
                    stderr =  that.o.node[`iconv-lite`].decode(Buffer.alloc(stderr.length,stderr, binaryEncoding), encoding);
                    output.push(StdOpt);
    ...
    */
    execSync(command){
        ...
        for(let i=0;i<command.length;i++){
            let cmd = command[i];
            try{
                let e = that.o.node.child_process.execSync(cmd, { encoding: binaryEncoding });
                e = e.toString("utf8");
                e = that.o.tool.string.trim(e,[`\n`,`\r`]);
                console.log(e);
                output.push(e);
            }catch(err){
                console.log("error:");
        ...
        if(command.length > 0 ){
            (function spawn_cmd(i){
                ...
                splitCmds.forEach((cmdOne)=>{
                    ...
                    if(_cmdOne){
                        _cmdOne = that.o.tool.string.trim(_cmdOne);
                        cmds.push(_cmdOne);
                    }
                });
                let
                    ...
                    parmas = cmds,
                    c = that.o.node.child_process.spawn(command_c,parmas)
                ;
        ...
        (function spawn_cmd(i){
            if(i >= command.length){
                ...
                let
                    ...
                    parmas = cmd.splice(1)
                ;
                that.o.node.child_process.execFile(command_c,parmas, (error, stdout, stderr) => {
                    if (error) {
                        console.log(error);
                    }
                    if (stderr) {
        ...
        (function spawn_cmd(i){
            if(i >= command.length){
                ...
                let
                    ...
                    parmas = cmd.splice(1)
                ;
                that.o.node.child_process.spawn(command_c,parmas);
                spawn_cmd(++i);
            }
        })(0);
    }
    ...
    */
    getParam(opt,paramName=null,defaultV=null,isValue=false,getKey=false){
        ...
        if(!paramName && paramName !== 0){//如果不传该值,则返回一个带有各类方法的类.!=0 防止正好取下标为0的参数。0刚好被识别为false
            ...
            for(let i=0;i<opt.length;i++){
                ...
                _vTnameNotDir = function(){
                    //排除该参数为一个目录
                    let Dir = that.o.node.path.parse(_vTname);
                    if(Dir.root && Dir.dir){
                        return false;
                    }
                    return true;
        ...
        for(let i=0;i<opt.length;i++){
            ...
            if(isValue){
                ...
                if(_v && _v.length > 1){
                    _v = _v[1];
                    _v = that.o.tool.string.trimX(_v);
                    let
                        _vIsInt = parseInt(_v)
                    ;
                    //如果是数字类型则直接返回数字型 
    ...
    exists(val){
        let
            ...
            exists = 0
        ;
        if(that.o.tool.array.isArray(val)){
            if(val.length){
                val.forEach((v)=>{
                   if(v) exists++;
                });
        ...
        }else if(that.o.tool.array.isObject(val)){
            for(let p in val){
                if(val[p]){
                    exists++;
                }
