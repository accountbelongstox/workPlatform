class fileC{
    ...
    */
    isFileSync(dir){
        ...
        let
            that = this
        ;
        if(that.load.node.fs.existsSync(dir) && that.load.node.fs.lstatSync(dir).isFile()){
            return true;
        }else{
            return false;
        }
    ...
    */
    writeFile(p,content,encodeObj,fn){
        ...
        that = this,
        encode = 'utf8',
        parentDir = that.load.node.path.join(p,"../")
        ;
        ...
        //转换编码
        content = that.load.node[`iconv-lite`].encode(content, encode);
        ...
        function writeF(){
            that.load.node.fs.writeFile(p,content, function (e) {
                if(e)console.log(e);
                if(fn)fn(e);
            });
        }
    ...
    */
    writeFileSync(p,content,opt){
        ...
        content = that.load.node["iconv-lite"].encode(content, encoding);
        that.load.node.fs.writeFileSync(p,content,opt);
    }
    ...
    */
    ...
    readFile(p){
        ...
        if(!that.load.node.fs.existsSync(p)){
            console.log(`(readFile) not find file...`);
            return null;
        }
        let t = that.load.node.fs.readFileSync(p,"utf8").toString();
        return t;
    }
