class installC{
	...
	run(){
        ...
        that.option.platform = that.o.config.platform;
	...
	*/
	getSoftInfo(softname){
        ...
        //名称
		info['softwareName'] = that.o.node.path.parse(info['downUrl']).base;
		//远程下载地址
	...
	*/
	...
	formatSoftwareUrl(softnameUrl){
        let 
        that = this,
        softDownUrlParse = that.o.node.url.parse(softnameUrl)
        ;
	...
	*/
	...
	localSoftwaerDir(softinfo){
        let 
        that = this,
        softdir = that.o.node.path.join(that.option.softwareSourceDir,`${softinfo.group}/${softinfo.softwareName}`)
        ;
        return softdir;
	}
    ...
    */
    getLocalSoftwaerOrNotDownload(softinfo,fn){
        ...
        if(that.o.tool.file.isFileSync(softinfo.softwareLocalDir)){
            if(fn)fn(softinfo.softwareLocalDir);
        }else{
        	let
        	saveDir = that.o.node.path.parse(softinfo.softwareLocalDir).dir
        	;
    ...
    */
    applicationDir(softInfo){
        ...
        for(let p in installToDir){
            ...
            if(that.o.tool.string.isString(installToDirOne)){

                installToDirRegArray.push(installToDirOne);

            }else if(that.o.tool.array.isObject(installToDirOne)){
        ...
        insatllDir = that.o.node.path.join(appBaseDir_,`${installPath}${softInfo.name}`);
        return insatllDir;
    }
    ...
    */
    isSetup(softDir){
        let
        that = this,
        files = that.o.node.fs.readdirSync(softDir),
        filePath = "",
        setup = that.o.tool.file.isOneExeFile(softDir)
        ;
    ...
    */
    isSetupX(softDir){
        ...
        ;
        if(!setup){
        	setup = that.o.tool.file.isOneExeFileX(softDir);
        }
        ...
        if(!setup){
        	...
        	files.forEach((file)=>{
        		if(that.o.tool.file.isExeFile(file)){
        			filesFullPath.push(that.o.node.path.join(softDir,file));
        	...
        	if(setupTmp)setup = that.o.node.path.join(softDir,setupTmp);
        }
        ...
        if(!setup){
        	...
        	filesFullPath.forEach((file)=>{
        		let
        		states = that.o.node.fs.statSync(file)
        		;
        		if( states.size > size){
        			indexOf = indexOfC;
        			setupTmp = file;
    ...
    */
    startInstall(softinfo,callback,applicationDirExists=false){
        ...
        if(!applicationDirExists && !softinfo.setupInstall){
            //将软件移到指定的目录
            that.o.tool.console.info(`rename  ${softinfo.tmp} => ${softinfo.applicationDir}...`,5);
            that.o.tool.file.rename(softinfo.tmp,softinfo.applicationDir,(data)=>{
                //下一步
    ...
    startInstall_next(softinfo,callback){
        ...
        if(softinfo.setupInstall){
            ...
            let 
                ...
                //softdirParse = that.o.node.path.parse(softdir),
            ...
            if(/\s+/.test(softdir)){
                ...
                ;
                if(!that.o.tool.file.existsSync(newSoftDir)){
                    that.o.tool.file.rename(softdir,newSoftDir,(data)=>{
                        //进入下一步查找安装文件
            ...
            that.o.tool.console.info(`setEnvironment...`,5);
    ...
    readyQueryInstallFile(softdir,softinfo,callback){
        ...
        that.queryInstallFile(softdir,softinfo,(installFile)=>{
            if(installFile){
                ...
                that.o.func.install.executeMisfileOrExefile(installFile,softinfo,()=>{
                ...
                ;
                that.o.tool.func.spawn(optCmd,(info)=>{
                    that.o.tool.console.info(`open application in ${softinfo.tmp}`,6);
                    that.getProgramInstallCallback(softinfo,callback);
                });
            }else{
                errInfo = `Not find setup.exe or entry application..`;
                that.o.tool.console.error(errInfo);
    ...
    */
    OnlyOneFolderIsSuperior(target,callback){
        let
        that = this,
        isOneFolder = that.o.tool.file.isOneFolder(target),
        folderParent = null
        ;
        if(isOneFolder){
        	folderParent = that.o.node.path.join(isOneFolder,`../`);
        	//将文件夹下的文件提升一级
        	console.log(`OnlyOneFolderIsSuperior ...`)
        	that.o.tool.file.rename(isOneFolder,folderParent,(data)=>{
        		//OnlyOneFolderIsSuperior
                if(that.o.tool.file.isOneFolder(folderParent)){
    ...
    */
    OnlyOneFolderIsDesc(softinfo,callback){
        ...
        //不是一个文件夹时才下降
        if(!isOneFolder){
            folderParent = that.o.node.path.join(target,`../__${softinfo.name}`);
            //先将文件移到另一个临时目录
            console.log(`OnlyOneFolderIsDesc ...`);
            let
                newFolder = that.o.node.path.parse(softinfo.downUrl).name,
                newFolderDesc = that.o.node.path.join(target,newFolder)
            ;
            that.o.tool.file.rename(target,newFolderDesc,(data)=>{
                //从临时目录将文件下降一级
                if(callback)callback();
            });
        }else{
    ...
    */
    setEnvironment(softinfo,callback){
        let
            ...
            envQueue = [],
            isNormalProgram = that.o.tool.file.isNormalProgram(softinfo.applicationDir),
        ...
        if(that.o.tool.array.isObject(environmentVariable)){
                ...
                if(isChangeVersion){
                    oneEvns.forEach((theOne)=>{
                        let
                            ...
                            oneEvnTmp = that.o.tool.file.pathFormat(softinfo.applicationDir,theOne),
                            versions = that.o.node.fs.readdirSync(softinfo.applicationDir)
                        ;
                        //将版本号排序
                        versions.sort((a,b)=>{
                            return b>a;
                        ...
                        oneEvnTmp = that.o.tool.file.pathFormat(oneEvnTmp);
                        //如果连接后的地址是相等的,则代表指定的环境变量是当前目录
                        //但因为此软件可以改变版本号,因此需要将最高版本号的当前目录设置为环境变量
                        if(oneEvnTmp === that.o.tool.file.pathFormat(softinfo.applicationDir)){
                            oneEvnTmp = that.o.node.path.join(softinfo.applicationDir,versions[0]);
                        }
                        // 不存在时,则向下继续查找
                        if(!that.o.tool.file.isDirSync(oneEvnTmp)){
                            //以查找到的第一个为准
                            versions.forEach((version)=>{
                                let
                                    versionPath = that.o.node.path.join(softinfo.applicationDir,version),
                                    queryEnvPathOne = that.o.tool.file.queryPathSync(versionPath,theOne)
                                ;
                                //查找到,中断循环
                                if(queryEnvPathOne){
                    ...
                    oneEvns.forEach((theOne)=>{
                        oneSetObjTmpArray.push( that.o.node.path.join(softinfo.applicationDir,theOne) );
                    });
                }
        ...
        that.o.tool.windows.setEvn(envQueue,()=>{
            //获取APP文件,建立快捷方式
            that.o.tool.console.info(`CreateShortcutsInApplication...`,5);
    ...
    */
    CreateShortcutsInApplication(softinfo,callback){
        ...
        if(typeofIs === "string" && application){
            that.o.tool.console.info(`application is a String.`,4);
            let
            ...
            currentShortcutItem.shortcutType = that.o.tool.array.getObjectTrue((new shortcutType()),true);
            applicationList.push(currentShortcutItem);
        }else if(typeofIs === "object" && application instanceof Array  && application.length){
            that.o.tool.console.info(`application is a Array.`,4);
            ...
            that.o.tool.console.info(`application is a Object.`,4);
            for(let p in application){
                let
                    app = application[p],
                    apps = []
                ;
                that.o.tool.array.isArray(app) ? apps = app : apps = [app];
                for(let i = 0;i<apps.length;i++){
                    ...
                    currentShortcutItem.shortcutType = that.o.tool.array.getObjectTrue(currentShortcutType,true);
                    //如果该软件自定了是否启动,以定义的为准
        ...
        applicationList.forEach((entry,index)=>{
            for(let m in entry){
                ...
                ;
                if(waitQueryItem && that.o.tool.string.isString(waitQueryItem)){
                    if(waitQueryItem.match(/\%.+?\%/)){
                        ...
                        for(let l in softinfo){
                            ...
                            if(waitQueryItem.includes(queryL)){
                                if(that.o.tool.string.isString(queryResult)){
                                    applicationList[index][m] = waitQueryItem.replace(queryL,queryResult);
                                }else{
                                    that.o.tool.console.waring(`${waitQueryItem.application} ${queryL} not is a string , replace fail.`);
                                }
                            }
                        }
                    }
            ...
            if(entry.application){
                that.o.tool.console.info(`Query ${softinfo.applicationDir} to ${entry.application} path.`,4);
                that.o.tool.console.info(`Query ${softinfo.applicationDir} to ${entry.icon} path.`,4);
                console.log(entry.icon);
                let
                    app_ = that.o.tool.file.queryFileSync(softinfo.applicationDir,entry.application),
                    icon_ = that.o.tool.file.queryFileSync(softinfo.applicationDir,entry.icon)
                ;
        ...
        if(applicationList.length > 0 ){
            that.o.tool.windows.createShortcut(applicationList,()=>{
                //检查是否是启动项,是否需要设置
                that.getProgramInstallCallback(softinfo,callback);
            });
        }else{
    ...
    */
    queryInstallFile(dir,softinfo,callback){
        ...
        let
            that = this,
            application = softinfo.application,
            isOneExeFile = that.o.tool.file.isOneExeFile(dir),
        ...
        if(application === null || application === false ){
            ...
            if(applicationExists ){
                applicationExists = that.o.tool.file.queryFileSync(dir,application);
            }
            /*-------------------------------------------------------------------*/
            if(applicationExists){
                ...
                if(isOneExeFile){
                    ...
                    let
                        files = that.o.tool.file.scanGetFileSync(dir),
                        exeFiles = [],
                        setupFileSize = 0,
                    ...
                    exeFiles.forEach((file)=>{
                        let
                            name = that.o.node.path.parse(file).name
                        ;
                        //1.优先查找带setup的文件
                        ...
                        //3.查找文件最大的项
                        let
                            stat = that.o.node.fs.lstatSync(file)
                        ;
                ...
                if(!setupFiles){
                    setupFiles = that.o.tool.file.queryFileSync(dir,`setup.exe`);
                }
    ...
    executeMisfileOrExefile(softdir,softinfo,callback){
        ...
        ;
        if(isMsi){
            that.o.tool.func.exec(softdir,(info)=>{
                if(callback && exeCallback){
                    exeCallback = false;
                    that.o.tool.console.success(`Install exited with child_process exec.`);
                    that.keyCodeActions(softinfo,callback);
                }
            });
        }else{
            exeInstall = that.o.node.child_process.spawn(softdir);
            exeInstall.stdout.on('data', (data) => {
                that.o.tool.console.info(data.toString(),4);
            });
            exeInstall.stderr.on('data', (data) => {
                that.o.tool.console.error(data.toString());
            });
            exeInstall.on('exit', (code) => {
                if(callback && exeCallback){
                    exeCallback = false;
                    that.o.tool.console.success(`Install exited with code ${code}`);
                    that.keyCodeActions(softinfo,callback);
                }
            });
            exeInstall.on('close', (code) => {
                if(callback && exeCallback){
                    exeCallback = false;
                    that.o.tool.console.success(`Install close with code ${code}`);
                    that.keyCodeActions(softinfo,callback);
                }
            });
        }
    ...
    */
    getProgramInstallCallback(softinfo,callback){
        ...
        that.createProgramInstallCallback(softinfo,(callbackName)=>{
            ...
            callbackClass.option.tmpDir = that.o.tool.file.getTmpDir();
            callbackClass.option.getTmpDir = function(pathname=``){
                return that.o.tool.file.getTmpDir(pathname)
            };
            ...
            if(configCallback){
                ...
                ;
                that.o.tool.module.runModule(`command`,opt,(option)=>{
                    //程序的安装结束将在回调集里结束
                    callbackClass.run(()=>{
                        that.keyCodeActions(softinfo,callback);
                    });
    ...
    */
    keyCodeActions(softinfo,callback){
        ...
        ;
        if(Activators){
            that.o.tool.console.info(`${softinfo.name} Activator...`,4);
            ...
            (function GetActivator(ALen){
                if(ALen >= Activators.length){
                    ...
                    let
                        ...
                        ActivatorScanRoots = that.o.node.fs.readdirSync(ActivatorRootDir),
                        ActivatorPathnameDir = that.o.node.path.join(ActivatorRootDir,GNPathname),
                        ActivatorResults = []
                    ;
                    ActivatorScanRoots.forEach((ActivatorScanFile,index)=>{
                        let
                            ActivatorScanFilePath = that.o.node.path.join(ActivatorRootDir,ActivatorScanFile)
                        ;
                        if(!that.o.tool.file.isFileSync(ActivatorScanFilePath)){
                           ActivatorScanRoots.splice(index,1);
                        }
                    });
                    that.o.tool.file.mkdirSync(ActivatorPathnameDir);
                    //如果是文字, 则可能为激活码或者路径
                    ...
                    (function ActivatorExec(len){
                        if(len >= ActivatorResults.length){
                            ...
                            let
                                ...
                                ActivatorDir = that.o.node.path.join(ActivatorPathnameDir,ActivatorName)
                            ;
                            ActivatorScanRoots.forEach((ActivatorScanFile)=>{
                                ...
                                if(ActivatorScanFile.includes(ActivatorName) || ActivatorName.includes(ActivatorScanFile)){
                                    let
                                        ActivatorScanFilePath = that.o.node.path.join(ActivatorRootDir,ActivatorScanFile)
                                    ;
                                    if(!that.o.tool.file.isFileSync(ActivatorDir)){
                                        //that.o.node.fs.createReadStream(ActivatorScanFilePath).pipe(that.o.node.fs.createWriteStream(ActivatorDir));
                                        that.o.node.fs.renameSync(ActivatorScanFilePath,ActivatorDir);
                                    }
                                }
                            });
                            //如果是路径,则直接执行
                            if(that.o.tool.file.isZipFile(ActivatorDir,true)){
                                let
                                    zipOption = [
                                        "zip",
                                        `--xtmp`,
                                ...
                                that.o.tool.module.runModule(`command`,zipOption,(option)=>{
                                    let
                                    ...
                                    if(ActivatorApplication){
                                        ActivatorApplicationPath = that.o.tool.file.queryFileSync(option.target,ActivatorApplication);
                                    }
                                    if(ActivatorApplicationPath){
                                        that.o.tool.console.info(`Activator in ${ActivatorApplicationPath}`,4);
                                        that.o.tool.func.exec(ActivatorApplicationPath,()=>{
                                            that.o.tool.func.exec(optCmd,()=>{
                                                GetActivator(++ALen);
                                            });
                                        });
                                    }else{
                            ...
                            }else if(that.o.tool.file.isExeFile(ActivatorDir)){
                                that.o.tool.func.execFileSync(ActivatorDir,()=>{
                                    GetActivator(++ALen);
                                });
                            }else{
                                //如果是文字,则为激活码
                                //需要写入文件,并打开
                                let
                                    keyCodePath= that.o.node.path.join(ActivatorPathnameDir,`${softinfo.name}KeyCode.txt`),
                                    optCmd = `notepad "${keyCodePath}"`
                                ;
                                that.o.tool.file.writeFile(keyCodePath,ActivatorName,()=>{
                                    that.o.tool.func.exec(optCmd,()=>{
                                        GetActivator(++ALen);
                                    });
                                });
    ...
    */
    createProgramInstallCallback(softinfo,callback){
        let
            ...
            callbackName = that.o.node.path.join(softinfo.callbackBaseDir,callbackPathname),
            callbackNameDir = that.o.node.path.parse(callbackName).dir,
            callbackClass = that.createProgramInstallCallbackContent(softinfo)
    	;
        that.o.tool.console.info(`Callback : ${callbackPathname}`,2);
    	that.o.tool.file.mkdir(callbackNameDir,()=>{
			that.o.node.fs.exists(callbackName,(exists)=>{
	        	if(!exists){
	        		that.o.node.fs.writeFile(callbackName,callbackClass,"utf8",(err)=>{
